...
void DrawScene() {
	...
	modelMat = glm::rotate(modelMat,glm::radians(y_rotate),glm::vec3(0.0f,1.0f,0.0f));  // Y축 회전
	modelMat = glm::rotate(modelMat,glm::radians(x_rotate),glm::vec3(1.0f,0.0f,0.0f));  // X축 회전
...
void TimerFunction(int value)
{
	if(x_start==1) x_rotate+=1.0f;
	if(x_start==2) x_rotate-=1.0f;
	if(y_start==1) y_rotate+=1.0f;
	if(y_start==2) y_rotate-=1.0f;
