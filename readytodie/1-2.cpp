...
void main(int argc,char** argv)
{
	...
	arr[0]={-1.0f,-1.0f,0,0,0.5,1.0,0.2};
	arr[1]={0,-1.0f,1.0f,0,0.5,0.5,0.1};
	arr[2]={-1.0f,0,0,1.0f,0.3,0.2,0.7};
	arr[3]={0,0,1.0f,1.0f,0.7,0.1,1.0};
	copy(arr.begin(),arr.end(),carr.begin());
...
void Mouse (int button,int state,int x,int y)
{
	...
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN){
		cout<<(((float)x/(SIZE/2))-1.0f)<<" "<<(((SIZE - (float)y)/(SIZE/2)) -1.0f)<<endl;
		for(int i=0;i<4;++i){
			...
			else if(carr[i].x1<(((float)x/(SIZE/2))-1.0f)&&carr[i].y1<(((SIZE - (float)y)/(SIZE/2)) -1.0f)&&carr[i].x2>(((float)x/(SIZE/2))-1.0f)&&carr[i].y2>(((SIZE - (float)y)/(SIZE/2)) -1.0f)) {
				f=dis(gen);
				l=dis(gen);
				o=dis(gen);
				break;
