...
#define CUBE_SIZE_1 0.05f
#define CUBE_SIZE_2 0.1f
#define CUBE_SIZE_3 0.15f
#define SPHERE_SIZE 0.1f
...
GLvoid drawScene() {
	...
	// uniform location 가져오기
	GLuint modelLoc = glGetUniformLocation(shaderProgramID,"model");
	GLuint viewLoc = glGetUniformLocation(shaderProgramID,"view");
	GLuint projLoc = glGetUniformLocation(shaderProgramID,"proj");
	GLuint faceColorLoc = glGetUniformLocation(shaderProgramID,"faceColor");
	...
	for(int i = 0; i < shapes.size(); i++) {
		if(shapes[i].object_num <= 3 + sphere_cnt) {
			...
			// 색상 설정 - 상자의 경우 vertex 데이터에서 색상 추출
			if(shapes[i].object_num == 0) {
				// 상자: vertex 데이터에서 첫 번째 정점의 색상을 가져와서 faceColor로 설정
				if(shapes[i].vertex.size() >= 6) {
					float r = shapes[i].vertex[3]; // 첫 번째 정점의 R
					float g = shapes[i].vertex[4]; // 첫 번째 정점의 G  
					float b = shapes[i].vertex[5]; // 첫 번째 정점의 B
					glUniform3f(faceColorLoc,r,g,b);
				} else {
					glUniform3f(faceColorLoc,1.0f,1.0f,1.0f); // 기본값
				}
			} else if(shapes[i].object_num == 1) {
				glUniform3f(faceColorLoc,0.8f,0.2f,0.15f); // 빨간색 큐브
			} else if(shapes[i].object_num == 2) {
				glUniform3f(faceColorLoc,0.2f,0.8f,0.15f); // 초록색 큐브
			} else if(shapes[i].object_num == 3) {
				glUniform3f(faceColorLoc,0.2f,0.2f,0.8f);  // 파란색 큐브
			} else if(shapes[i].object_num >= 4) {
				glUniform3f(faceColorLoc,0.4f,0.2f,0.6f);  // 보라색 구체들
			}
