...
uniform_real_distribution<float> rlocate(-2.0f,2.0f);
...
void InitData(){
	...
	shape.push_back(Shape(model[2],1));
	shape.back().t={0.0f,0.0f,0.0f};

	shape.push_back(Shape(model[1],1));
	shape.back().s={0.2f,0.2f,0.2f};
	shape.back().t={1.0f,0.0f,0.0f};
	shape.push_back(Shape(model[1],1));
	shape.back().s={0.5f,0.5f,0.5f};
	shape.back().t={2.0f,0.0f,0.0f};
	shape.push_back(Shape(model[1],1));
	shape.back().s={1.0f,1.0f,1.0f};
	shape.back().t={3.0f,0.0f,0.0f};

	shape.push_back(Shape(model[0],1));
	shape.back().s={5.0f,0.0f,5.0f};
	shape.back().t={0.0f,-0.5f,0.0f};

	shape.push_back(Shape(model[0],1));
	shape.back().s={0.5f,0.5f,0.5f};
	shape.back().t={0.0f,5.0f,-5.0f};
...
void DrawScene() {
	...
	GLint ambientLocation = glGetUniformLocation(shaderProgramID,"ambientLight"); //--- viewPos 값 전달: 카메라 위치




	glm::vec3 lightPos={0.0f,0.0f,0.0f};
	glm::mat4 lightModelMat = shape[5].modelMat;
...
void Keyboard(unsigned char key,int x,int y)
{
	...
	if(s){
		for(int i=0;i<50;++i){
			shape.push_back(Shape(model[0],1));
			shape.back().s={0.1f,0.1f,0.1f};
			shape.back().t={rlocate(gen),20.0f,rlocate(gen)};
			shape.back().speed=static_cast <float> (rcolor(gen));
		}
	}
	else{
		shape.erase(shape.begin()+6,shape.end());
	}
	break;
	}
...
void TimerFunction(int value)
{
	...
	for(size_t i=0;i<shape.size();++i){
		...
		if((i>=1&&i<=3)||(r_&&i==5)){
			shape[i].angle+=glm::radians((i!=5?shape[i].t.x:2.0f)*0.5f*2.0f);
		}
		if(i>=6){
			shape[i].t.y -= shape[i].speed;
			if(shape[i].t.y < 0.0f){
				shape[i].t.y = 20.0f;
				shape[i].t.x = rlocate(gen);
				shape[i].t.z = rlocate(gen);
				shape[i].speed = static_cast <float> (rcolor(gen));
			}
		}
		ori=glm::rotate(ori,shape[i].angle,glm::vec3(0.0f,1.0f,0.0f));
